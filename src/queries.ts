...
export const getMovieShowtimes = async (params: QueryParams) => {
  ...
  if (dayName === null) {
    ...
    sql = `SELECT f.nome, f.classificacao, p.status, p.data_estreia, p.semana_inicio, p.semana_fim, p.segunda, p.terca, p.quarta, p.quinta, p.sexta, p.sabado, p.domingo
      FROM programacao p
    ...
    if (validDays.length > 0) {
      sql = `SELECT f.nome, f.classificacao, p.status, p.data_estreia, p.semana_inicio, p.semana_fim, ${buildDaySelection(
        validDays
      )}
        FROM programacao p
        JOIN filmes f ON p.id_filme = f.id
    ...
    // Dia específico
    sql = `SELECT f.nome, f.classificacao, p.status, p.data_estreia, p.semana_inicio, p.semana_fim, p.${dayName}
      FROM programacao p
      JOIN filmes f ON p.id_filme = f.id
      JOIN cinemas c ON p.id_cinema = c.id
      WHERE c.id = $1
